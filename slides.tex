...
Write allowed by compiler, but observable difference
  ...
  \begin{frame}
    ...
    Measure how much code is \const{}, and any changes

    \vspace{2em}
    Found the more widely used the API, the more \const{}
  \end{frame}

  \begin{frame}
    \frametitle{Attempt 2}

    Find any commits removing/adding \const{}

    \vspace{2em}
    Minor number of removals/additions

    \vspace{2em}
    Big removal since an object was immutable (no mutable methods)
  \end{frame}

  \begin{frame}
    \frametitle{Attempt 3}

    Initial static analysis, way too many false positives
  \end{frame}

  \begin{frame}
    \frametitle{Personal Frustration}

    LLVM Pass Framework is geared for transformations, not analysis

    \vspace{2em}
    In analysis, all APIs are mutable (okay, you can add \const{} manually)
  ...
  \begin{frame}
    \frametitle{\const{} as a Newbie Guard}
